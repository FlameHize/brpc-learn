...
namespace brpc {
// The global EventDispatcher is responsible for listening to the events on the fd. 
// If there are already bthreads on the fd that are being processed, return directly. 
// Otherwise, start a bthread to process. The bthread started by the EventDispatcher 
// is responsible for reading the messages on the fd. Every time it reads one, 
// start the bthread to process it. The last one is executed in place until it is read. 
// Because the EventDispatcher itself is neither responsible for reading messages 
// nor processing messages, The throughput can be very large. Messages between fds 
// and within fds can be concurrent. All threads are wait free, which enables brpc 
// to process messages from different sources in a timely manner under high load
