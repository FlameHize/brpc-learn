...
// The most important thing is to register all kinds of protocol support, 
// including http. In short, it specifies the message parsing function 
// of each protocol, the request processing function used by the server, 
// and the response processing function used by the client

// For example, the request processing function. In the following figure, 
// the http protocol is taken as an example. 
// ProcessHttpRequest is responsible for processing the received http request. 
// In this function, the request will be processed and then handed over 
// to the user-defined function for processing
