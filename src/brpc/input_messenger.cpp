...
static void QueueMessage(InputMessageBase* to_run_msg,
    ...
    if (bthread_start_background(
        ...
        // Call the process of the corresponding protocol_ Request to process the messages from the cut
...
void InputMessenger::OnNewMessages(Socket* m) {
    ...
    // OK in most cases.
    // First Create,last_msg.release() is NULL because of unique_ptr
    std::unique_ptr<InputMessageBase, RunLastMessage> last_msg;
    bool read_eof = false;
    // Keep reading until it is finished
    while (!read_eof) {
        ...
        while (1) {
            size_t index = 8888;
            // messenger is an InputMessenger object, which is responsible for 
            // cutting and processing messages from fd
            // InputMessenger will try the callback of the user specified protocols one by one. 
            // When a Parse successfully cuts the next message, it will call the corresponding ProcessXXX. 
            // Since there is usually only one message format on a connection, 
            // InputMessenger will record the last selection to avoid repeated attempts every time
            ...
            // Start a bthread to execute ProcessInputMessage to process an msg. 
            // If n messages are read from a fd consecutively (n>1), 
            // InputMessenger will start n-1 bthreads to process the first n-1 messages respectively, 
            // and the last message will be processed in place
            ...
            m->PostponeEOF();
            // Assign Message processing function and pointer
            // Msg->_ Process is the request processing function of the corresponding protocol, 
            // in which the user function will eventually be called
            // Msg->_ process refers to the ProcessXXXRequest during protocol registration
            ...
            // Can't continue read,store the last_msg
            // then the last_msg can use itself deleter to read last one
