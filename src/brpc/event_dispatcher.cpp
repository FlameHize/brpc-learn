    ...
    while (!_stop) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (e[i].events & (EPOLLIN | EPOLLERR | EPOLLHUP)
                ...
                // After the server is started, the EventDispatcher 
                // will listen to epoll events. As a server, 
                // it receives a new connection belonging to epoll_ In event, 
                // Socket:: StartInputEvent will be called for processing
