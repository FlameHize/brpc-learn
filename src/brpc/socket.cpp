...
void* Socket::ProcessEvent(void* arg) {
    ...
    // The event occurs on the listening port, when a new connection request is received,
    // the callback function _ on_ edge_ triggered_ Events will be called when a new connection come 
    // In fact it is the OnNewConnection assigned in StartAccept 
...
// If no:
//    old_head is fully written, set _write_head to NULL and return true--verifcation;
//    old_head is not written yet, keep _write_head unchanged and return false--verifcation;
...
/**
 * @brief example: new requests 1 2 3 appended after old request 0
 * 
 *          (old_head)                      (pre_head)     (_write_head)
 *  NULL <--- request0 <---  request1  <---  request2  <---  request3  
 *                          
 *                          After Inverst Link
 *                                                (old_head)    (_write_head)
 * request 0  ---> request 1  ---> request2  --->  request3  <--- request4   
 *                                                    |
 *                                                   NULL
 */
bool Socket::IsWriteComplete(Socket::WriteRequest* old_head,
    ...
    // Represents the data to be written next
    WriteRequest* desired = NULL;
    bool return_when_no_more = true;
    // old_head->data.empty() == false that's represent data has not wirte finished
    ...
    // this.compare_exchange_strong(expected, desired, std::desired,memeory_order)
    // Atomic comparison * this and expect values. If they are equal, 
    // replace the former with desired and return true
    // otherwise, load the *this's value to expected and return false
    if (_write_head.compare_exchange_strong(
            new_head, desired, butil::memory_order_acquire)) {
        // _write_head == new_head -> _write_head = desired
    ...
    // _write_head != new_head -> expected(new_head) = _write_head
    ...
    do {
        // Wait for the added node to be set next
...
int Socket::ConnectIfNot(const timespec* abstime, WriteRequest* req) {
    ...
    ReAddress(&s);
    req->socket = s.get();
    // Use KeepWriteIfConnected as a callback function to register epoll out events
    if (_conn) {
...
int Socket::StartWrite(WriteRequest* req, const WriteOptions& opt) {
    ...
    // A = B.exchange(C,std::memory_order)
    //      -Assign the value before A exchange to B
    //      -Assign the value of C to A
    // Socket::Socket() set the _write_head to NULL
    WriteRequest* const prev_head =
        _write_head.exchange(req, butil::memory_order_release);
    // After this exchange operation, prev_head = _write_head; _write_head = req
    if (prev_head != NULL) {
    ...
    // If the writing is finished, write once in place within the current thread.  
    // Other,start another thread to write before it is finished
    
    // Iswritecomplete is used to jude whether the writing is completed
    // and invert the Linklist
...
KEEPWRITE_IN_BACKGROUND:
    ...
    // use KeepWrite function to set a bthread continue write
 ...
 *                         After Inverst Link
 *   (req)
 * request 0  ---> request 1  ---> request2  --->  request3  ---> NULL  
 * 
 */
void* Socket::KeepWrite(void* void_arg) {
    ...
    do {
        ...
        // Release completed requests in batches
        ...
        // Find the tail of the current linked list. For example, in the above case, 
        // the tail is request 3. Use this tail as a parameter to call Iswritecomplete function
        if (NULL == cur_tail) {
            for (cur_tail = req; cur_tail->next != NULL;
                 cur_tail = cur_tail->next);
        }
        // Return when there's no more WriteRequests and req is completely written.
        // The newly added request will be added here, and the cur will be updated_ tail

        // (1) req == cur_ Tail, there has new req
        // There was only one request to write, but there was a new request, so compare_ exchange_ Strong Failed. 
        // Update the new tail to cur after flipping_ Tail, return false, unfinished

        // (2) req == cur_ Tail, no new req
        // Originally, there was only one to write, and there was no new one. If this one was not finished, 
        // false is returned, and when it was finished, true is returned

        // (3) req !=  cur_ Tail, there is a new req
        // Because !singular_ node is established, so return_ when_ no_ more=false, 
        // but there are new requests, so compare_ exchange_ Strong Failed, so the linked list will be 
        // flipped to ensure that the newly added request becomes a new tail after flipping 
        // and is updated to cur_ tail

        // (4) req !=  cur_ tail, no new req
        // Because !singular_ node is established, so return_ when_ no_ more = false, 
        // and compare_ exchange_ Strong Successful. Return directly to return_ when_ no_ More, that is false
...
// Only responsible for judging whether there is a bthread on the fd where the event occurs. 
// If there is no bthread, start it in place and return it directly
int Socket::StartInputEvent(SocketId id, uint32_t events,
    ...
    if (s->_nevent.fetch_add(1, butil::memory_order_acq_rel) == 0) {
        ...
        // This function starts the bthread and will release the current bthread
        // Official document: "EventDispatcher gives up its pthread to the new bthread, 
        // so that it has a better cache location and can read the data on the fd as soon as possible". 
        // There is no need for pthread switching to help the performance ". 
        // If for some reason bthread_ start_ If the urgent fails to start, execute the ProcessEvent in place
