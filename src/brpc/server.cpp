...
Acceptor* Server::BuildAcceptor() {
    ...
    std::vector<Protocol> protocols;
    // Get all the protocols supported by registration
    ListProtocols(&protocols);
    for (size_t i = 0; i < protocols.size(); ++i) {
        ...
        // Traverse these protocols and add them through AddHandler which handles messages. 
        // Note that the handler here All processes are processes in protocols [i]_ Request, 
        // that is, the corresponding protocol is used by the server to process the received request. 
        // If it is used by the client, it is process_ response
...
// Add the internal implementation of the service
