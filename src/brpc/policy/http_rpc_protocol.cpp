...
void ProcessHttpRequest(InputMessageBase *msg) {
    ...
    // Get the pointer to server
    // msg->_ Arg is the arg of the handler. The arg of the handler 
    // is actually the pointer to start the server. It is assigned 
    // when various handlers are added to BuildAcceptor()
    ...
    // Core Process: Use the MethodProperty to get the service and method
