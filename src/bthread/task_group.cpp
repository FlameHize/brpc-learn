...
bool TaskGroup::wait_task(bthread_t* tid) {
    do {
        ...
        // Wait for notification to terminate blocking
        // When the blocking ends, execute step_task() to steal work. If the theft is successful, return true
        _pl->wait(_last_pl_state);
        // Steal Priority  Current TaskGroup's remote rq Other TaskGroup's rq  Other TaskGroup's remote_rq 
        if (steal_task(tid)) {
            return true;
        }
#else
...
void TaskGroup::run_main_task() {
    ...
    bthread_t tid;
    // wait_task: Find a task. It will involve work staling (Load balancing for each CPU core)
    while (wait_task(&tid)) {
        // Switch the runtime context of stacks, registers, and restore the context of the next task
    ...
    , _remote_nsignaled(0)
{
    ...
    // Select one PL from the four PLs of TaskControl and assign it to TaskGroup
    // All TGs under TC are divided into four groups. Each group shares a PL, 
    // which regulates the production and consumption of bthread tasks between TGs
...
void TaskGroup::task_runner(intptr_t skip_remained) {
    ...
    do {
        ...
        void* thread_return;
        try {
            // Really run current bthread's callback function
        ...
        // wake up all the thread joiner
        butex_wake_except(m->version_butex, 0);

        g->_control->_nbthreads << -1;
        g->set_remained(TaskGroup::_release_last_context, m);
        // Find the next task and switch to its corresponding runtime context
        // From rq and remote_of_rq to fetch TaskGroup tasks. If no other TG tasks can be found, 
        // set _cur_ Meta is _main_ Tid, that will let the loop of runner ()  terminated. 
        // Then it will return to run_ main_ task loop function, continue to wait_ Task() is waiting for a new task
        ending_sched(&g);
...
void TaskGroup::sched_to(TaskGroup** pg, TaskMeta* next_meta) {
    ...
    if (__builtin_expect(next_meta != cur_meta, 1)) {
        ...
        // Switch tls_bls
        // tls_bls represents local storage in TM (bthread). Restore first, 
        // and assign the value to the local storage of the next TM. Then execute jump_ stack() to switch stacks
...
// In the thread without woker (TG), the bthread can only be queued to the 
// remote_queue of the TG with worker thread
void TaskGroup::ready_to_run_remote(bthread_t tid, bool nosignal) {
    // Add a mutex lock to remote_rq of the current TaskGroup. Then join the bthread to remote_rq
    _remote_rq._mutex.lock();
    while (!_remote_rq.push_locked(tid)) {
        // Execute flush_nosignal_tasks_remote_locked() if you fail to join the team, 
        // sleep for 1ms, and then try to join the team again
        // The only reason for failing to join the team is that the capacity of (remote_rq) is full. 
        // The operation of flush_nosignal_tasks_remote_locked is nothing more than sending a signal 
        // to make the task (TM/bthread) in remote_rq be consumed as soon as possible. Leave room for new missions
    ...
    // After the while ends. It means that the new task has been enlisted
    if (nosignal) {
        ...
        _remote_rq._mutex.unlock();
        // Notify others to consume that (1 + additional_signal) nums task
        _control->signal_task(1 + additional_signal);
    }
}
