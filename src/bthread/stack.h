...
struct StackStorage {
    ...
    // Anonymously map a space through mmap, and then assign the high altitude address to bottom
    // Point to the bottom of the coroutine stack
    void* bottom;
...
/**
 * @brief Allocate a piece of stack
 * 
 * @param s StackStorage's pointer
 * @param stacksize Stack's size
 * @param guardsize Protection Page's size
 * @return int 
 */
...
struct ContextualStack {
    // Point to the top of the coroutine stack
    bthread_fcontext_t context;
    // Indicates the type (size) of the stack
    StackType stacktype;
    // Stack space
