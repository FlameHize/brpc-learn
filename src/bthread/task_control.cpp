...
void* TaskControl::worker_thread(void* arg) {
    // The processing before TG creation is also the callback g_worker_start_fun
    // The function can be used to perform operations through bthread_set_worker_Startfn() 
    // to set this callback function
    run_worker_startfn();    
#ifdef BAIDU_INTERNAL
    logging::ComlogInitializer comlog_initializer;
#endif
    // Get the pointer of TaskControl
    TaskControl* c = static_cast<TaskControl*>(arg);
    // Create thread_local param TaskGroup
    ...
    // Initialize thread_local TaskGroup* pointer
    tls_task_group = g;
    // worker counts puls 1
    c->_nworkers << 1;
    // while(1) loop -TaskGroup run main task
    g->run_main_task();

    // Return status information at the end of TaskGroup
    stat = g->main_stat();
...
void TaskControl::signal_task(int num_task) {
    ...
    // Now num_task indicates the number of tasks that need to be wakened but are not wakened
    if (num_task > 0) {
    ...
    // If there are still tasks left (indicating that consumers are not enough), 
    // and the concurrency of the global TC is less than that configured in the gflag bthread_min_concurrency), 
    // then it will call add_worker(1) to increase the number of workers
    if (num_task > 0 &&
